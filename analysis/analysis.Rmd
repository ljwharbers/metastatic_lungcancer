...
plt6 = ggplot(percentage_altered, aes(x = location, y = percentage, color = CNA)) +
  ...
  labs(y = "% of genome with SCNAs", x = "", title = "Percentage of genome altered")
...
percentage_altered = dcast(percentage_altered, sample + subtype ~ location, value.var = "percentage", fun.aggregate = sum)
number_altered = dcast(number_altered, sample + subtype ~ location, value.var = "N", fun.aggregate = sum)
...
# Right side barplot
mat_typed = mat
for(x in colnames(mat_typed)) {
  mat_typed[[x]] = gsub("AMP|DEL", annot$subtype[annot$sample == x], mat_typed[[x]])
}
dt_fractions = data.table(gene = rownames(mat_typed), 
                          fraction_adeno = rowSums(mat_typed == "ADENOCARCINOMA") / sum(annot$subtype == "ADENOCARCINOMA"),
                          fraction_squamous = rowSums(mat_typed == "SQUAMOUS CELL CARCINOMA") / sum(annot$subtype == "SQUAMOUS CELL CARCINOMA"))
dt_fractions = melt(dt_fractions)

dt_fractions[, gene := factor(gene, levels = rev(rownames(mat_typed)[row_order(plt17)]))]
dt_fractions[, variable := factor(variable, levels = c("fraction_squamous", "fraction_adeno"))]

# Plot
plt17_rightside = ggplot(dt_fractions, aes(x = gene, y = value, fill = variable)) +
  geom_col() +
  scale_y_continuous(expand = c(0, 0)) +
  scale_fill_npg() +
  labs(x = "", y = "Fraction of samples AMP/DEL") +
  coord_flip() +
  theme(axis.ticks.y = element_blank())

plt17_rightside
png("./plots/fig1h_rightsideannot_Arm_level_SCNAs_oncoprint.png", width = 18, height = 12, units = "in", res = 200)
plt17_rightside
dev.off()
```
...
plt18 = plot_grid(dendro, heatmap,  align = "h", rel_widths = c(0.2, 2), ncol = 2)
...
plt19 = ggplot(total_cor, aes(x = names, y = cor_vec, color = names)) + 
...
# plot
plt20 = ggplot(total, aes(x = variable, y = value, color = variable)) +
  geom_boxplot(outlier.shape = NA) +
...
plt21 = ggplot(total, aes(x = subtype, y = value, color = variable)) +
  geom_boxplot(outlier.shape = NA) +
...
# Set sample order
setcolorder(mat_genes, c('gene', levels(annot$sample)))
...
plt22 = oncoPrint(mat_genes_subset,
           alter_fun = list(
             background = alter_graphic("rect", width = 0.9, height = 0.9, fill = "#FFFFFF", col = "black"),
             AMP = alter_graphic("rect", width = 0.85, height = 0.85, fill = cols["AMP"]),
             DEL = alter_graphic("rect", width = 0.85, height = 0.85, fill = cols["DEL"])), 
...
# Make right side annotation bars
mat_genes_subset_typed = mat_genes_subset
for(x in colnames(mat_genes_subset_typed)) {
  mat_genes_subset_typed[[x]] = gsub("AMP|DEL", annot$subtype[annot$sample == x], mat_genes_subset_typed[[x]])
}
dt_fractions = data.table(gene = rownames(mat_genes_subset_typed), 
                          fraction_adeno = rowSums(mat_genes_subset_typed == "ADENOCARCINOMA") / sum(annot$subtype == "ADENOCARCINOMA"),
                          fraction_squamous = rowSums(mat_genes_subset_typed == "SQUAMOUS CELL CARCINOMA") / sum(annot$subtype == "SQUAMOUS CELL CARCINOMA"))
dt_fractions = melt(dt_fractions)

dt_fractions[, gene := factor(gene, levels = rev(rownames(mat_genes_subset_typed)[row_order(plt22)]))]
dt_fractions[, variable := factor(variable, levels = c("fraction_squamous", "fraction_adeno"))]

# Plot
plt22_rightside = ggplot(dt_fractions, aes(x = gene, y = value, fill = variable)) +
  geom_col() +
  scale_y_continuous(expand = c(0, 0)) +
  scale_fill_npg() +
  labs(x = "", y = "Fraction of samples AMP/DEL") +
...
plt22_rightside
png("./plots/fig3a_rightsideannot_CNA_genes_discovery_cohort_oncoprint.png", width = 7, height = 18, units = "in", res = 200)
plt22_rightside
dev.off()
...
plt23 = ggplot(total, aes(x = qvalue_prim, y = qvalue_met, labels = gene, color = variable)) +
  geom_point() +
...
dev.off()
```
```{r gene level analysis of discovery cohort using GISTIC2 (ADENO), include=TRUE}
# Load in gene-level GISTIC2 data
prim_amp = fread("./data/GISTIC2/primary_adeno/amp_genes.conf_90.txt",
                 drop = 1)
prim_del = fread("./data/GISTIC2/primary_adeno/del_genes.conf_90.txt",
                 drop = 1)
met_amp = fread("./data/GISTIC2/metastasis_adeno/amp_genes.conf_90.txt",
                 drop = 1)
met_del = fread("./data/GISTIC2/metastasis_adeno/del_genes.conf_90.txt",
                 drop = 1)
cosmicplus = fread("./data/cosmicplus.tsv")
...
plt23a = ggplot(total, aes(x = qvalue_prim, y = qvalue_met, labels = gene, color = variable)) +
  geom_point() +
  geom_vline(xintercept = 0.05, linetype = 2, color = "red") +
  geom_hline(yintercept = 0.05, linetype = 2, color = "red") +
  scale_color_brewer(palette = "Set1") +
...
plt23a
png("./plots/adeno_gistic_scores_discoverycohort.png", width = 8, height = 8, units = "in", res = 200)
plt23a
dev.off()
```

```{r gene level analysis of discovery cohort using GISTIC2 (SQUAMOUS), include=TRUE}
# Load in gene-level GISTIC2 data
prim_amp = fread("./data/GISTIC2/primary_squamous/amp_genes.conf_90.txt",
                 drop = 1)
prim_del = fread("./data/GISTIC2/primary_squamous/del_genes.conf_90.txt",
                 drop = 1)
met_amp = fread("./data/GISTIC2/metastasis_squamous/amp_genes.conf_90.txt",
                 drop = 1)
met_del = fread("./data/GISTIC2/metastasis_squamous/del_genes.conf_90.txt",
                 drop = 1)
cosmicplus = fread("./data/cosmicplus.tsv")
...
plt23b = ggplot(total, aes(x = qvalue_prim, y = qvalue_met, labels = gene, color = variable)) +
  geom_point() +
  geom_vline(xintercept = 0.05, linetype = 2, color = "red") +
  geom_hline(yintercept = 0.05, linetype = 2, color = "red") +
  scale_color_brewer(palette = "Set1") +
...
plt23b
png("./plots/squamous_gistic_scores_discoverycohort.png", width = 8, height = 8, units = "in", res = 200)
plt23b
dev.off()
```
...
# Plot
plt24 = ggplot(counts, aes(x = discovery, y = validation)) +
  geom_point(size = 3) +
...
plt27 = ggplot(muts_subtype, aes(x = subtype, y = muts, color = subtype)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.2) +
  ylab("TMB") +
...
plt28 = ggplot(muts_sync, aes(x = Type_of_metastases, y = muts, color = Type_of_metastases)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.2) +
  ylab("TMB") +
...
plt29 = cowplot::plot_grid(plt, annot_plt, ncol = 1, align = "v", axis = "lr", rel_heights = c(1, 0.05))
...
plt30 = ggplot(type_occurrences, aes(x = variable, y = value, color = variable)) + 
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.2) +
...
plt31 = cowplot::plot_grid(plt, legend, rel_heights = c(3, 0.2), ncol = 1)
...
plt32 = ggplot(dt_smoking, aes(x = Smoking, y = Contribution, color = Smoking)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = .25) +
  scale_color_npg() +
...
# Right side barplot
mat_subset_typed = mat_subset
for(x in colnames(mat_subset_typed)) {
  mat_subset_typed[[x]] = gsub("non_.*|frame.*", annot$value[annot$sample == x], mat_subset_typed[[x]])
}
dt_fractions = data.table(gene = rownames(mat_subset_typed), 
                          fraction_adeno = rowSums(mat_subset_typed == "ADENOCARCINOMA") / sum(annot$value == "ADENOCARCINOMA"),
                          fraction_squamous = rowSums(mat_subset_typed == "SQUAMOUS CELL CARCINOMA") / sum(annot$value == "SQUAMOUS CELL CARCINOMA"))
dt_fractions = melt(dt_fractions)

dt_fractions[, gene := factor(gene, levels = rev(rownames(mat_subset_typed)[row_order(plt)]))]
dt_fractions[, variable := factor(variable, levels = c("fraction_squamous", "fraction_adeno"))]

# Plot
plt33_rightside = ggplot(dt_fractions, aes(x = gene, y = value, fill = variable)) +
  geom_col() +
  scale_y_continuous(expand = c(0, 0)) +
  scale_fill_npg() +
  labs(x = "", y = "Fraction of samples AMP/DEL") +
...
plt33_rightside
png("./plots/fig4d_rightsideAnnot_mutated_genes_oncoprint.png", width = 7, height = 14, units = "in", res = 200)
plt33_rightside
dev.off()
...
# Combine plots
plt34 = cowplot::plot_grid(plt, annot_plt, ncol = 1, align = "v", rel_heights = c(1, 0.05))
...
ego = enrichGO(gene = data_subs$ENTREZID,
               ...
               qvalueCutoff = 0.05,
               readable = T)
plt35 = dotplot(ego, showCategory = 20)
...
# Plot
plt36 = ggplot(dt, aes(x = V1.x, y = V1.y)) +
  geom_point(size = 2) +
...
plt37 = ggplot(dt[gene %in% drivers$gene, ], aes(x = V1.x, y = V1.y)) +
  geom_point(size = 2) +
  labs(y = "Gene panel normalized frequency", x = "WES normalized frequency") +
  geom_smooth(method = "lm", se = F, linetype = 2, color = "red", size = 1.5) +
  stat_cor(method = "spearman")
